...
roadmapRoutes.post(
	...
	async (c) => {
		...
		const { records } = await db.execute<{ exists: boolean }>(
			sql`select ${exists(
				db
					.select({ n: sql`1` })
					.from(roadmapsTable)
		...
		const sq = db.$with("select_max_order").as(
			db
				.select({
					order: sql`coalesce(${max(roadmapToMilestone.order)}, 0) + 1`.as(
						"order"
					),
				})
				.from(roadmapToMilestone)
				.where(eq(roadmapToMilestone.roadmapId, id))
		);
		...
		await db
			...
			.values({
				roadmapId: id,
				milestoneId,
				order: sql`(select ${sq.order} from ${sq})`,
			})
			.catch(handleDbError);
...
roadmapRoutes.get("/:id", async (c) => {
	...
	const records = await db
		.select({
			...
			milestone: {
				...
				estTime: milestonesTable.estTime,
			},
		})
		.from(roadmapsTable)
		...
		.orderBy(roadmapToMilestone.order)
		.catch(handleDbError);
	...
	const milestones = milestone
		? records.map((record) => record.milestone)
		: [];
